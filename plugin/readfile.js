...
var text={
	...
	f1_0:"Welcome to the EO-Lab course for Deep Learning for EO data processing on the EO Cloud with Tensorflor. You've already noticed that this course is designed in such a way that you can study the material on your own. All relevant documents, papers, and other external sources are provided. The content is conveyed through interactive sessions, meaning that chapters as well as exercises use multi-media content to  study the matter presented. Explore and enjoy!",
	...
	f4_0:"Chapter 1: Introduction to artificial intellegence for earth observation" ,
		//3.1 CNNs
		f4_1:"The general objective is to classify land cover in satellite imagery ussing convolutional neural networks. Also called CNNs.",
			f4_1_f_1:"There are different appraoches applied by deep learning. Supervised learning, which requires images and labels of the objects to be classified. They are feeded to a machine learning algorithm, which then provides the classification.",
			f4_1_f_2:"Supervised machine learning works with an artificial intellegence model. This model is trained with image and label data, based on this data the parameter of an algorthim are adjusted. The model learns and improves through training. Afterwards the model can then be sued to predict new images.",
		...
		//3.2 Classification tasks
			...
			f4_2_f_2:"Segmentation first creates image segments, which are used for object detection and classification. Semantic or image segmentation in contrast to instance segmentation assigns class probabilites to each pixel. Providing a very detailed prediction",
		//3.3 DL workflow
		f4_3:"Let us take a look at the general deep learning workflow for earth observation data. At first the input data has to be collected. There are different possibilites how to get input data, which will be described closer in the next slide.",
			...
			f4_3_f_4:"If the classification is statisfying it can be used further. In case of a low prediction result new input data has to be collected to improve the model performance.",
	...
	f5_0:"Chapter 2: The Convolutional Neural Network architecture ",
		...
		f5_2:"An example of a deep learning model is the U-Net model, which will be used in the example. The U-Net model architecture has the shape of an U and is acombination of an encoder and decoder.",
	...
	f6_0:"Chapter 3: Application example with tensorflow",
		...
		//Preparation
		f6_2:"At first use the EO- browser to identify the necessary satellite image. Then go to your virtual machine and search for the selected satellite image in the repositories provided by the EO-Lab. Define two AOIs one for training and the other one for testing the model. It is important that all classes are represented in the training image. Then store subsets based on your AOIs of the sentinel 2 image and the ESA worldcover. This will be your training data. It is important, that the image raster in this case the sentinel 2 image. and the label raster, here the ESA Worldcover,  have the same extantion. The image and raster data can be prepared in a GIS software to also ensure a continours numbering of the classes. Starting by zero.",
		//Libaries
		f6_3:"Now, open a new Jupyter notebook. The follwing libaires need to be installed.",
		...
		// Sample Prepartion-2
		f6_5:"Now the image and label patches are seperated into the training and validation data. This figure shows how the image patches with the according classification can look like.",
		// Model Preparation
	...
	//6 Summary
		...
		f7_1_f_3:"Which steps on your Vitual Machine on EO-Lab are necessary to prepare EO data for analysis with a DL mode",
		f7_1_f_4:"Which processing components to include in a TensorFlow script for perfoming semantic segmentation with a U-Net model",
